...
export const useTasks = () => {
  const { tasks, isInitialized, setTasks, getSortedTasks } = useTaskStore();
  ...
  useEffect(() => {
    if (query.data && !isInitialized) {
      console.log('Setting tasks in store:', query.data.length);
      setTasks(query.data);
    }
  ...
  // Debug logging
  useEffect(() => {
    console.log('useTasks state:', {
      queryData: query.data?.length,
      isInitialized,
      tasksInStore: tasks.length,
      isLoading: query.isLoading
    });
  }, [query.data, isInitialized, tasks.length, query.isLoading]);
